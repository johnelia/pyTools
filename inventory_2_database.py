...
def save_to_mysql(dataframe):
    ...
    print(f"DB has been bitchslapped...")
    try:
        ...
        for index, row in dataframe.iterrows():
            try:
                cursor.execute("""
                    INSERT INTO installed_software (name, version, publisher, install_date, date_ran)
                    VALUES (%s, %s, %s, %s, %s)
                """, (row['Name'], row['Version'], row['Publisher'], row['InstallDate'], current_date))
